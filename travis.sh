...
if [ -n "${COVERAGE}" ]; then
  # build (linux)
  run_all " -DBUILD_CFP=ON -DBUILD_ZFPY=ON -DBUILD_ZFORP=ON -DZFP_WITH_ALIGNED_ALLOC=1 -DBUILD_OPENMP=ON -DBUILD_CUDA=OFF -DWITH_COVERAGE=ON"
else
  ...
  run_all " -DBUILD_CFP=ON -DCFP_NAMESPACE=cfp2 -DBUILD_ZFPY=ON -DBUILD_ZFORP=${BUILD_ZFORP} -DZFP_WITH_ALIGNED_ALLOC=1 -DBUILD_OPENMP=OFF -DBUILD_CUDA=OFF"
