...
void CheckMemberVarsCopied(array2f arr1, array2f arr2)
{
...
void CheckDeepCopyPerformed(array2f arr1, array2f arr2, uchar* arr1UnflushedBitstreamPtr)
{
...
TEST_P(Array2fTest, given_compressedArray_when_copyConstructor_then_memberVariablesCopied)
{
  array2f arr(inputDataSideLen, inputDataSideLen, getRate(), inputDataArr, 128);
...
TEST_P(Array2fTest, given_compressedArray_when_copyConstructor_then_deepCopyPerformed)
{
  // create arr with dirty cache
  array2f arr(inputDataSideLen, inputDataSideLen, getRate(), inputDataArr);
...
TEST_P(Array2fTest, given_compressedArray_when_setSecondArrayEqualToFirst_then_memberVariablesCopied)
{
  array2f arr(inputDataSideLen, inputDataSideLen, getRate(), inputDataArr, 128);
...
TEST_P(Array2fTest, given_compressedArray_when_setSecondArrayEqualToFirst_then_deepCopyPerformed)
{
  // create arr with dirty cache
  array2f arr(inputDataSideLen, inputDataSideLen, getRate(), inputDataArr);
  uchar* arrUnflushedBitstreamPtr = arr.compressed_data();
...
int main(int argc, char* argv[]) {
  ...
  static_cast<void>(::testing::AddGlobalTestEnvironment(new Array2fTestEnv));
