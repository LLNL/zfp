...
static int setup(void **state)
{
  ...
  void* bs = NULL;
  bundle -> container . stream = _prefixFOR(zforp_stream_open)(&bs);
//  bundle -> container . stream = _prefixFOR(zforp_stream_open)((bitstream *)((void *)0));
   *state = bundle;
  return 0;
}
...
static int teardown(void **state)
{
  ...
  _prefixFOR(zforp_stream_close)(&bundle -> container);
  free(bundle);
  return 0;
}
...
// default values imply expert mode
  assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_expert);
}
...
// ensure this config would be rejected by zfp_stream_set_params()
  ...
  assert_int_equal((_prefixFOR(zforp_stream_set_params)(&bundle -> container,&minbits,&stream -> maxbits,&stream -> maxprec,&stream -> minexp)),0);
  stream -> minbits = stream -> maxbits + 1;
  assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_null);
}

static void setNonExpertMode(zfp_struct_container *containerPtr)
{
  uint val = 64 - 2;
  _prefixFOR(zforp_stream_set_precision)(containerPtr,&val);
  assert_int_not_equal((_prefixFOR(zforp_stream_compression_mode)(containerPtr)),zfp_mode_expert);
}
...
static void setDefaultCompressionParams(zfp_struct_container *containerPtr)
{
  ...
  uint maxprec = 64;
  int minexp = -1074;
  assert_int_equal((_prefixFOR(zforp_stream_set_params)(containerPtr,&minbits, &maxbits, &maxprec, &minexp)),1);
  assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(containerPtr)),zfp_mode_expert);
}
    ...
    for (dims = 1; dims <= 4; dims++) {
      for (rate = 1; rate <= ((zfpType % 2?32 : 64)); rate++) {
        for (wra = 0; wra <= 1; wra++) {
          ...
          _prefixFOR(zforp_stream_set_rate)(&bundle -> container,&r,&zfpType,&dims,&wra);
          zfp_mode mode = _prefixFOR(zforp_stream_compression_mode)(&bundle -> container);
...
static void given_zfpStreamSetWithFixedPrecision_when_zfpStreamCompressionMode_expect_returnsFixedPrecisionEnum(void **state)
{
  ...
  for (prec = 1; prec < 64; prec++) {
    setDefaultCompressionParams(&bundle -> container);
    _prefixFOR(zforp_stream_set_precision)(&bundle -> container,&prec);
    zfp_mode mode = _prefixFOR(zforp_stream_compression_mode)(&bundle -> container);
...
static void given_zfpStreamSetWithMaxPrecision_when_zfpStreamCompressionMode_expect_returnsExpertModeEnum(void **state)
{
  ...
  setDefaultCompressionParams(&bundle -> container);
  uint val = 64;
  _prefixFOR(zforp_stream_set_precision)(&bundle -> container,&val);
  assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_expert);
}
...
static void given_zfpStreamSetWithFixedAccuracy_when_zfpStreamCompressionMode_expect_returnsFixedAccuracyEnum(void **state)
{
  ...
  for (accExp = 1023; accExp > - 1074 && ldexp(1.,accExp) != 0.; accExp--) {
    ...
    _prefixFOR(zforp_stream_set_accuracy)(&bundle -> container,&acc);
    zfp_mode mode = _prefixFOR(zforp_stream_compression_mode)(&bundle -> container);
...
static void given_zfpStreamSetWithExpertParams_when_zfpStreamCompressionMode_expect_returnsExpertEnum(void **state)
{
  ...
  uint maxprec = 52u;
  int minexp = -1000;
  assert_int_equal((_prefixFOR(zforp_stream_set_params)(&bundle -> container,&minbits, &maxbits, &maxprec, &minexp)),1);
  assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_expert);
}
...
static void given_zfpStreamDefaultModeVal_when_zfpStreamSetMode_expect_returnsExpertMode_and_compressParamsConserved(void **state)
{
  struct setupVars *bundle = ( *state);
  zfp_stream *stream = bundle -> container . stream;
  uint64 mode = _prefixFOR(zforp_stream_mode)(&bundle -> container);
  uint minbits = stream -> minbits;
  uint maxbits = stream -> maxbits;
  uint maxprec = stream -> maxprec;
  int minexp = stream -> minexp;
  setNonExpertMode(&bundle -> container);
  assert_int_equal((_prefixFOR(zforp_stream_set_mode)(&bundle -> container,&mode)),zfp_mode_expert);
    ...
    for (dims = 1; dims <= 4; dims++) {
      for (rate = 1; rate <= ((zfpType % 2?32 : 64)); rate++) {
        for (wra = 0; wra <= 1; wra++) {
          double r = (double)rate;
          _prefixFOR(zforp_stream_set_rate)(&bundle -> container,&r,&zfpType,&dims,&wra);
          assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_fixed_rate);
          uint64 mode = _prefixFOR(zforp_stream_mode)(&bundle -> container);
          uint minbits = stream -> minbits;
          uint maxbits = stream -> maxbits;
          uint maxprec = stream -> maxprec;
          int minexp = stream -> minexp;
          setDefaultCompressionParams(&bundle -> container);
          zfp_mode zfpMode = _prefixFOR(zforp_stream_set_mode)(&bundle -> container,&mode);
...
static void given_zfpStreamSetPrecisionModeVal_when_zfpStreamSetMode_expect_returnsFixedPrecision_and_compressParamsConserved(void **state)
{
  ...
  for (prec = 1; prec < 64; prec++) {
    _prefixFOR(zforp_stream_set_precision)(&bundle -> container,&prec);
    assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_fixed_precision);
    uint64 mode = _prefixFOR(zforp_stream_mode)(&bundle -> container);
    uint minbits = stream -> minbits;
    uint maxbits = stream -> maxbits;
    uint maxprec = stream -> maxprec;
...
static void given_fixedPrecisionMaxPrecModeVal_when_zfpStreamSetMode_expect_returnsExpert_and_compressParamsConserved(void **state)
{
  ...
  val = 64 - 2;
  _prefixFOR(zforp_stream_set_precision)(&bundle -> container,&val);
  assert_int_not_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_expert);
  assert_int_equal((_prefixFOR(zforp_stream_set_mode)(&bundle -> container,&mode)),zfp_mode_expert);
...
static void given_zfpStreamSetAccuracyModeVal_when_zfpStreamSetMode_expect_returnsFixedAccuracy_and_compressParamsConserved(void **state)
{
  ...
  for (accExp = 1023; accExp > - 1074 && ldexp(1.,accExp) != 0.; accExp--) {
    double acc = ldexp(1., accExp);
    _prefixFOR(zforp_stream_set_accuracy)(&bundle -> container,&acc);
    assert_int_equal((_prefixFOR(zforp_stream_compression_mode)(&bundle -> container)),zfp_mode_fixed_accuracy);
    uint64 mode = _prefixFOR(zforp_stream_mode)(&bundle -> container);
    uint minbits = stream -> minbits;
    uint maxbits = stream -> maxbits;
    uint maxprec = stream -> maxprec;
...
// reset params
  ...
  int minexp = -1074;
  assert_int_equal((_prefixFOR(zforp_stream_set_params)(&bundle -> container,&minbits, &maxbits, &maxprec, &minexp)),1);
  assert_int_equal((_prefixFOR(zforp_stream_set_mode)(&bundle -> container,&mode)),expectedMode);
...
static void given_invalidCompressParamsModeVal_when_zfpStreamSetMode_expect_returnsNullMode_and_paramsNotSet(void **state)
{
  ...
  uint maxprec = 52u;
  int minexp = -1000;
  assert_int_equal((_prefixFOR(zforp_stream_set_params)(&bundle -> container,&minbits, &maxbits, &maxprec, &minexp)),0);
