...
_catFunc3(given_, CFP_ARRAY_TYPE, _header_expect_matchingMetadata)(void **state)
{
  ...
  assert_int_equal(CFP_NAMESPACE.header.dimensionality(srcCfpHdr), 1);
  assert_int_equal(CFP_NAMESPACE.SUB_NAMESPACE.rate(srcCfpArr), CFP_NAMESPACE.header.rate(srcCfpHdr));
  assert_int_equal(CFP_NAMESPACE.SUB_NAMESPACE.size(srcCfpArr), CFP_NAMESPACE.header.size_x(srcCfpHdr));
...
_catFunc3(given_, CFP_PTR_TYPE, _when_distance_expect_correct)(void **state)
{
  ...
  assert_int_equal(CFP_NAMESPACE.SUB_NAMESPACE.pointer.distance(cfpArrPtrA, cfpArrPtrB), (int)cfpArrPtrB.reference.i - (int)cfpArrPtrA.reference.i);
