...
_catFunc3(given_, CFP_ARRAY_TYPE, _when_setIJK_expect_entryWrittenToCacheOnly)(void **state)
{
  ...
  cfp_api.SUB_NAMESPACE.set_ijk(cfpArr, 1, 1, 1, (SCALAR)VAL);
...
_catFunc3(given_, CFP_ARRAY_TYPE, _when_getIJK_expect_entryReturned)(void **state)
{
  ...
  CFP_ARRAY_TYPE* cfpArr = bundle->cfpArr;
  uint i = 1, j = 2, k = 1;
  cfp_api.SUB_NAMESPACE.set_ijk(cfpArr, i, j, k, (SCALAR)VAL);

  // dirty cache doesn't immediately apply compression
  assert_true(cfp_api.SUB_NAMESPACE.get_ijk(cfpArr, i, j, k) == (SCALAR)VAL);
}
