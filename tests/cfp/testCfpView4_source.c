...
_catFunc3(given_, CFP_VIEW_TYPE, _when_ctor_subset_expect_returnsNonNullPtr)(void **state)
{
  ...
  CFP_VIEW_TYPE cfpView = 
    CFP_NAMESPACE.VIEW_NAMESPACE.ctor_subset(cfpArr, OFFSET_X, OFFSET_Y, OFFSET_Z, OFFSET_W, 
...
static void
_catFunc3(given_, CFP_VIEW_TYPE, _when_globalw_expect_offsetMatches)(void **state)
{
  struct setupVars *bundle = *state;
  CFP_ARRAY_TYPE cfpArr = bundle->cfpArr;
  CFP_VIEW_TYPE cfpView = bundle->cfpView;
  ...
  assert_int_equal(CFP_NAMESPACE.VIEW_NAMESPACE.global_w(cfpView, offset), OFFSET_W+offset);
}
...
static void
_catFunc3(given_, CFP_VIEW_TYPE, _when_sizew_expect_sizeMatches)(void **state)
{
  struct setupVars *bundle = *state;
  CFP_ARRAY_TYPE cfpArr = bundle->cfpArr;
  CFP_VIEW_TYPE cfpView = bundle->cfpView;

  assert_int_equal(CFP_NAMESPACE.VIEW_NAMESPACE.size_w(cfpView), CFP_NAMESPACE.SUB_NAMESPACE.size_w(cfpArr));
}
...
_catFunc3(given_, CFP_VIEW_TYPE, _when_set_expect_getValueMatches)(void **state)
{
  ...
  CFP_NAMESPACE.VIEW_NAMESPACE.set(cfpView, i, j, k, l, val);
  assert_true(CFP_NAMESPACE.VIEW_NAMESPACE.get(cfpView, i, j, k, l) == val);
}
...
_catFunc3(given_, CFP_VIEW_REF_TYPE, _when_get_expect_entryReturned)(void **state)
{
  ...
  CFP_VIEW_REF_TYPE cfpViewRef = CFP_NAMESPACE.VIEW_NAMESPACE.ref(cfpView, i, j, k, l);
  CFP_NAMESPACE.VIEW_NAMESPACE.set(cfpView, i, j, k, l, VAL);
...
_catFunc3(given_, CFP_VIEW_REF_TYPE, _when_set_expect_viewUpdated)(void **state)
{
  ...
  assert_true(CFP_NAMESPACE.VIEW_NAMESPACE.get(cfpView, i, j, k, l) == (SCALAR)VAL);
}
...
_catFunc3(given_, CFP_VIEW_REF_TYPE, _when_copy_expect_viewUpdated)(void **state)
{
  ...
  CFP_NAMESPACE.VIEW_NAMESPACE.set(cfpView, i1, j1, k1, l1, VAL);
  CFP_VIEW_REF_TYPE cfpViewRef_a = CFP_NAMESPACE.VIEW_NAMESPACE.ref(cfpView, i1, j1, k1, l1);
  CFP_VIEW_REF_TYPE cfpViewRef_b = CFP_NAMESPACE.VIEW_NAMESPACE.ref(cfpView, i2, j2, k2, l2);
  CFP_NAMESPACE.VIEW_REF_NAMESPACE.copy(cfpViewRef_b, cfpViewRef_a);

  assert_true(CFP_NAMESPACE.VIEW_NAMESPACE.get(cfpView, i2, j2, k2, l2) == (SCALAR)VAL);
}
...
_catFunc3(given_, CFP_VIEW_PTR_TYPE, _when_get_set_expect_correct)(void **state)
{
  ...
  SCALAR val = 5;
  CFP_VIEW_PTR_TYPE cfpViewPtr = CFP_NAMESPACE.VIEW_NAMESPACE.ptr(cfpView, i, j, k, l);
...
_catFunc3(given_, CFP_VIEW_PTR_TYPE, _when_lt_expect_less)(void **state)
{
  ...
  size_t k1 = 1, k2 = 2;
  size_t l1 = 1, l2 = 2;
  CFP_VIEW_PTR_TYPE cfpViewPtrA = CFP_NAMESPACE.VIEW_NAMESPACE.ptr(cfpView, i1, j1, k1, l1);
  CFP_VIEW_PTR_TYPE cfpViewPtrB = CFP_NAMESPACE.VIEW_NAMESPACE.ptr(cfpView, i2, j2, k2, l2);
...
_catFunc3(given_, CFP_VIEW_PTR_TYPE, _when_distance_expect_correct)(void **state)
{
  ...
  assert_int_equal((int)CFP_NAMESPACE.VIEW_PTR_NAMESPACE.distance(cfpViewPtrA, cfpViewPtrB),
                   (int)(i2 +
                         ...
                         l2*CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView)*CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView)*CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView)) -
                   (int)(i1 +
                         ...
                         l1*CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView)*CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView)*CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView)));
...
_catFunc3(given_, CFP_VIEW_PTR_TYPE, _when_next_expect_correct)(void **state)
{
  ...
  size_t idx = (i + CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) * (j + CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView) * (k + CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView) * l))) + oi;
  size_t x = idx % CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView);
  size_t y = ((idx - x) / CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView)) %  CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView);
  size_t z = ((idx - y * CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) - x) / (CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView)))            % CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView); 
  size_t w = (idx - z * CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) - y * CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) - x)            / (CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView)); 

  assert_int_equal(cfpViewPtr.x, x);
  assert_int_equal(cfpViewPtr.y, y);
  assert_int_equal(cfpViewPtr.z, z);
  assert_int_equal(cfpViewPtr.w, w);
  assert_ptr_equal(cfpViewPtr.container, CFP_NAMESPACE.VIEW_NAMESPACE.ptr(cfpView, i, j, k, l).container);
}
...
_catFunc3(given_, CFP_VIEW_PTR_TYPE, _when_inc_expect_correct)(void **state)
{
  ...
  size_t idx = (i + CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) * (j + CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView) * (k + CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView) * l))) + 1; 
  size_t x = idx % CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView);
  size_t y = ((idx - x) / CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView)) %  CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView);
  size_t z = ((idx - y * CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) - x) / (CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView)))            % CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView); 
  size_t w = (idx - z * CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) - y * CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) - x)            / (CFP_NAMESPACE.VIEW_NAMESPACE.size_x(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_y(cfpView) * CFP_NAMESPACE.VIEW_NAMESPACE.size_z(cfpView)); 
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_ref_expect_correct)(void **state)
{
  ...
  assert_int_equal(cfpViewRef.w, 0);
}
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_ref_at_expect_correct)(void **state)
{
  ...
  assert_int_equal(cfpViewRef.w, 4);
}
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_ptr_expect_correct)(void **state)
{
  ...
  assert_int_equal(cfpViewPtr.w, 0);
}
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_ptr_at_expect_correct)(void **state)
{
  ...
  assert_int_equal(cfpViewPtr.w, 4);
}
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_dec_expect_correct)(void **state)
{
  ...
  assert_int_equal(cfpViewIter.w, 3);
}
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_next_expect_correct)(void **state)
{
  ...
  CFP_VIEW_ITER_TYPE cfpViewIter = CFP_NAMESPACE.VIEW_NAMESPACE.begin(cfpView);
  cfpViewIter = CFP_NAMESPACE.VIEW_ITER_NAMESPACE.next(cfpViewIter, 256);
  cfpViewIter = CFP_NAMESPACE.VIEW_ITER_NAMESPACE.next(cfpViewIter, 255);
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_prev_expect_correct)(void **state)
{
  ...
  CFP_VIEW_ITER_TYPE cfpViewIter = CFP_NAMESPACE.VIEW_NAMESPACE.begin(cfpView);
  cfpViewIter = CFP_NAMESPACE.VIEW_ITER_NAMESPACE.next(cfpViewIter, 511);
  cfpViewIter = CFP_NAMESPACE.VIEW_ITER_NAMESPACE.prev(cfpViewIter, 255);

  assert_ptr_equal(cfpViewIter.container, cfpView.object);
  assert_int_equal(cfpViewIter.x, 4);
  assert_int_equal(cfpViewIter.y, 0);
  assert_int_equal(cfpViewIter.z, 0);
  assert_int_equal(cfpViewIter.w, 0);
}
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_distance_expect_correct)(void **state)
{
  ...
  cfpViewIter1 = CFP_NAMESPACE.VIEW_ITER_NAMESPACE.next(cfpViewIter1, 255);
  cfpViewIter2 = CFP_NAMESPACE.VIEW_ITER_NAMESPACE.next(cfpViewIter2, 511);

  assert_int_equal(CFP_NAMESPACE.VIEW_ITER_NAMESPACE.distance(cfpViewIter1, cfpViewIter2), 256);
  assert_int_equal(CFP_NAMESPACE.VIEW_ITER_NAMESPACE.distance(cfpViewIter2, CFP_NAMESPACE.VIEW_NAMESPACE.begin(cfpView)), -511);
}
...
_catFunc3(given_, CFP_VIEW_ITER_TYPE, _when_get_index_expect_correct)(void **state)
{
  ...
  size_t l_idx = CFP_NAMESPACE.VIEW_ITER_NAMESPACE.l(cfpViewIter);
