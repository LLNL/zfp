...
public:
  ...
  ~Cache()
  {
    deallocate_aligned(tag);
    deallocate_aligned(line);
#ifdef ZFP_WITH_CACHE_PROFILE
  ...
  void resize(uint minsize)
  {
    ...
    reallocate_aligned(tag, ((size_t)mask + 1) * sizeof(Tag), 0x100);
    reallocate_aligned(line, ((size_t)mask + 1) * sizeof(Line), 0x100);
    clear();
  }
...
protected:
  ...
  {
    mask = c.mask;
    clone_aligned(tag, c.tag, mask + 1, 0x100u);
    clone_aligned(line, c.line, mask + 1, 0x100u);
#ifdef ZFP_WITH_CACHE_PROFILE
    hit[0][0] = c.hit[0][0];
    hit[0][1] = c.hit[0][1];
    hit[1][0] = c.hit[1][0];
