...
public:
  ...
  static const uint by = 16;                // number of blocks per tile along y
  static const uint bz = 16;                // number of blocks per tile along z
  static const uint block_size = 4 * 4 * 4; // number of scalars per block
...
protected:
  ...
  uint encode_block(zfp_stream* zfp, const Scalar* block, uchar shape = 0) const
  {
    ...
    uint bits = zfp::codec<Scalar>::encode_block_3(zfp, block, shape);
  ...
  uint decode_block(zfp_stream* zfp, Scalar* block, offset p, uchar shape = 0) const
  {
    ...
    uint bits = zfp::codec<Scalar>::decode_block_3(zfp, block, shape);
