...
public:
  ...
  BlockCache2(BlockStorage2<Scalar, Codec>& storage, size_t bytes = 0) : cache(bytes), storage(storage) {}
  ...
  void get_block(uint block_index, Scalar* p, ptrdiff_t sx, ptrdiff_t sy) const
  {
    ...
    if (line)
      line->get(p, sx, sy, storage.block_shape(block_index));
    else
      storage.decode(block_index, p, sx, sy);
  }
...
protected:
  ...
  {
    CacheLine* p = 0;
    uint block_index = storage.block_index(i, j);
  ...
  BlockStorage2<Scalar, Codec>& storage; // storage backed by cache
};
