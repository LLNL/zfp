...
public:
  ...
  // block storage for array of size nx and given rate
  BlockStorage1(uint nx, double rate) :
    nx(nx),
    bx((nx + 3) / 4),
    bits_per_block(0),
    data(0),
  ...
  // perform a deep copy
  void deep_copy(const BlockStorage1& s)
  {
    free();
    nx = s.nx;
    bx = s.bx;
  ...
  size_t encode(uint block_index, const Scalar* p, ptrdiff_t sx) const
  {
  ...
  size_t decode(uint block_index, Scalar* p, ptrdiff_t sx) const
  {
