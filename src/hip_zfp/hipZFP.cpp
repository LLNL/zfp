...
{
  if (hipZFP::is_gpu_ptr(field->data)) {
    ...
    if (internal::is_contiguous(dims, stride, offset)) {
      ...
      if (hipMalloc(&d_data, size) != hipSuccess)
        std::cerr << "failed to allocate device memory for field" << std::endl;
      if (hipMemcpy(d_data, h_data, size, hipMemcpyHostToDevice) != hipSuccess)
