...
namespace hipZFP
{
                            ...
                            int nstreams_chunk)
    {
        ...
        hipLaunchKernelGGL(copy_length, dim3(blocks), dim3(1024), 0, 0, bitlengths, chunk_offsets, first, nstreams_chunk);
    }
                                                ...
                                                int maxpad32)
    {
        ...
        HIP_DYNAMIC_SHARED(unsigned int, sm_in);              // sm_in[num_tiles * maxpad32]
        ...
        // Loop on all the bitstreams of the hiprrent chunk, using the whole resident grid.
